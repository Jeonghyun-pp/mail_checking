...
코드는 private 저장소에 올라가 있다:
`https://github.com/Jeonghyun-pp/mail_checking.git`

VM은 이 private 레포를 clone해야 하므로 **GitHub Personal Access Token**이
필요하다 (VM에는 브라우저 로그인이 없다).

1. https://github.com/settings/personal-access-tokens/new 접속
2. **Fine-grained token** 생성:
   - Repository access → *Only select repositories* → `mail_checking`
   - Permissions → Repository permissions → **Contents: Read-only**
   - 만료일 적당히 설정
3. 생성된 토큰(`github_pat_...`)을 복사해 둔다 — 4단계 clone에 쓴다.
...
```bash
# 사전 준비에서 만든 토큰을 URL에 넣어 clone (private 레포)
git clone https://<github_pat_토큰>@github.com/Jeonghyun-pp/mail_checking.git
cd mail_checking
...
> 토큰을 URL에 넣어 clone하면 이후 `git pull` 도 인증 없이 동작한다.
